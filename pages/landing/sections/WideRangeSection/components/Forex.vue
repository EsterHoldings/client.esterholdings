  ...
  <div class="forex h-[366px] w-full">
    <UiContainer class="border !p-0 h-full">
      <div
        ...
        <div class="forex__img-wrapper relative">
          <!-- Лоадер з трьох крапок до повного завантаження зображення -->
          <div
              v-show="!imgLoaded"
              class="dots-loader"
              aria-live="polite"
          ...
          <!-- Показуємо UiImage завжди, але робимо плавне проявлення після preload -->
          <UiImage
              :fitContain="true"
              :fitPosition="'right'"
              :src="imgSrc"
...
// Джерело зображення залишаємо як у тебе
const imgSrc = "/static/forex-money.webp";

// Стан завантаження для плавного показу та лоадера
const imgLoaded = ref(false);

// Без зміни UiImage: робимо попереднє завантаження через native Image(),
// щоб нічого в твоєму компоненті не ламати і не залежати від кастомних подій.
onMounted(() => {
  ...
  preloader.onerror = () => {
    // Навіть якщо помилка — ховаємо лоадер, щоб не зависав
    imgLoaded.value = true;
  };
...
.forex {
  ...
  &__img {
    ...
    object-fit: contain;
    width: 585px;
    z-index: 0; // нижче лоадера, але вище фонів
    transform: translate(0, 15px);
    overflow: hidden;

    // Початково приховано, щоб не мигтіло фоном
    opacity: 0;
...
.dots-loader {
  ...
  gap: 8px;
  z-index: 1; // поверх зображення, поки воно не проявилось
}
.dots-loader span {
  width: 8px;
  height: 8px;
...
@media (max-width: 991px) {
  ...
  .forex__img-wrapper {
    ...
    width: 100%;
    height: 280px;
    min-height: 280px; // зберігаємо висоту під час завантаження
    overflow: hidden;
  }
