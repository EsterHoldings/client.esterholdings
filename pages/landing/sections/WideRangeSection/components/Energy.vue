<template>
  <div class="energy h-[366px] w-full">
    <UiContainer class="border !p-0 h-full">
      <div
        class="wrapper rounded-[10px] h-full"
        :class="{ 'light-theme': themeStore.currentTheme === 'light' }">
        <div class="energy__content p-10 h-full">
          <UiTextH3 class="energy__title"> {{ t("landing.sections.wide_range_tabs.Energy.title") }}</UiTextH3>
          ...
          <div>
            <Nuxt-Link to="market-instruments?index=5">
              <UiButtonDefault state="primary"> {{ t("landing.sections.wide_range_tabs.Energy.btn") }}</UiButtonDefault>
            </Nuxt-Link>
          </div>
        </div>

        <div class="energy__img-wrapper">
          <UiImage
            :fitContain="true"
            :fitPosition="'right'"
            src="/static/energy_image.webp"
            class="energy__img" />
        </div>
      </div>
    </UiContainer>
