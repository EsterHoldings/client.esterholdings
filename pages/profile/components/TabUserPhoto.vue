  ...
  <div class="text-[var(--ui-text-main)]">
    <PanelDefault class="mx-auto text-center p-5 w-[350px]">
      <div class="flex flex-col items-center gap-4">
        <div
          v-if="previewUrl"
          class="relative w-[300px] h-[300px] rounded-[10px] bg-[var(--ui-background)] border border-[var(--color-stroke-ui-dark)] overflow-hidden">
          <UiImage
            :src="previewUrl"
          ...
          <button
            ...
            @click="remove"
            :aria-label="t('cabinet.profile.components.tab-user-photo.buttons.delete_aria')"
            type="button">
            ×
          </button>
        </div>
        ...
        <div
          v-else
          class="relative w-[300px] h-[300px] rounded-[10px] bg-[var(--ui-background)] border border-[var(--color-stroke-ui-dark)] flex items-center justify-center group cursor-pointer"
          @click="clickSelectionFile">
          <span class="text-7xl text-gray-400">👤</span>

          <div
            class="absolute left-0 right-0 bottom-0 h-[35%] w-full p-2.5 flex items-center justify-center bg-[var(--ui-background-secondary)] rounded-b-[10px] opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div
          v-if="loading"
          class="w-full h-1.5 bg-[var(--ui-primary-main-defalt)] rounded-md overflow-hidden">
          <div
        ...
        <UiTextSmall>{{ t("cabinet.profile.components.tab-user-photo.hints.formats") }}</UiTextSmall>
        <UiTextSmall>{{ t("cabinet.profile.components.tab-user-photo.hints.maxSize") }}</UiTextSmall>
      </div>

      <UiHorizontalLine class="my-5" />

      <UiTextH5 class="mb-5">{{ t("cabinet.profile.components.tab-user-photo.verification.title") }}</UiTextH5>
      <div class="flex items-center justify-center gap-2">
        <span><UiIconSuccess /></span>
        <UiTextSmall>{{ t("cabinet.profile.components.tab-user-photo.verification.approved") }}</UiTextSmall>
      </div>
    </PanelDefault>
  </div>
</template>
    ...
    try {
      ...
      error.value =
        extractApiErrorMessageWithTranslator(
          e,
          resolveText,
          t("cabinet.profile.components.tab-user-photo.messages.upload_error")
        ) || t("cabinet.profile.components.tab-user-photo.messages.upload_error");
    } finally {
      loading.value = false;
    }
    ...
    try {
      ...
      error.value =
        extractApiErrorMessageWithTranslator(
          e,
          resolveText,
          t("cabinet.profile.components.tab-user-photo.messages.delete_error")
        ) || t("cabinet.profile.components.tab-user-photo.messages.delete_error");
    } finally {
      loading.value = false;
    }
